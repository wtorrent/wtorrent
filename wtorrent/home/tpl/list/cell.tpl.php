<div class="tbBulk" id="itab{$hash}">
	...
	<div id="tbCell" style="cursor: pointer; background-color: #e5edf4; border-top: 0px solid #d4d4d4;" onmouseover="style.backgroundColor='#d5e991';" onmouseout="style.backgroundColor='#e5edf4'">	
		<div id="tbContentCell" style="width: 96px; padding-left: 0px; padding-top: 6px; height: 24px; text-align: left;">
			...
			{if $web->getState($hash) eq 0 || $web->getOpen($hash) eq 0}<div style="cursor: pointer; display: inline;"><img src="{$DIR_IMG}bullet_go.png" onclick="command('start', '{$hash}');" alt="{$str.start}" title="{$str.start}" /></div>&nbsp;{else}<div style="cursor: pointer; display: inline;"><img src="{$DIR_IMG}cross.png"  onclick="command('stop', '{$hash}');" alt="{$str.stop}" title="{$str.stop}" /></div>&nbsp;{/if}{if $web->getOpen($hash) neq 0}<div style="cursor: pointer; display: inline;"><img src="{$DIR_IMG}lock_delete.png" onclick="command('close', '{$hash}');" alt="{$str.close}" title="{$str.close}" />&nbsp;</div>{/if}<div style="cursor: pointer; display: inline;"><img src="{$DIR_IMG}delete.png" onclick="command('erase', '{$hash}');" alt="{$str.erase}" title="{$str.erase}" /></div>&nbsp;{if $web->isHashChecking($hash) neq true}<div style="cursor: pointer; display: inline;"><img src="{$DIR_IMG}c_hash.png" onclick="command('chash', '{$hash}');" alt="{$str.chash}" title="{$str.chash}" /></div>{/if}
		</div>
